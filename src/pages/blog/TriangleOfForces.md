...
Over some years I have developed a potentially interesting point of view that suggest another way to use _CSS-the-good-bits_ in blissful harmony with CSS-in-JS for a reasonable and flexible styling solution. Most of this article looks at historical and modern contexts for attempts to "tame" CSS using a mental model to support the reasoning behind the practical bit at the end and [part 2](/TriangleOfForces-part2.md) provides more in depth and practical examples on how these ideas enhance the re-use of components
...
First I need to get this off my chest... I really like CSS 😲 (yes, I remember web development before CSS). I think it is an amazingly good and simple language for what it does, and what it does, it mostly does in the best possible way:
...
4. It is declarative (caveats), simple and easy to understand (caveats) which mean it is low barrier to entry (boundary)
...
And wasteful. And confusing. There is plenty said about the shortfalls of CSS; "append only" giant stylesheets, 10 selector deep rules, CSS mirroring HTML structure...

Nevertheless if we look closely, largely speaking all these problems - the hard bits of CSS - arise from the following 3 things (in descending order of impact):

1. All style declarations are in the same, global scope - there are no inherent boundaries
2. Browsers don't always play by the same rules - though, this is barely a consideration these days!
3. Sometimes, the nuances of specificity can throw up some unexpected results. 🤔...

This first point is both a part of the genius of CSS and the foot-gun by which we can make our lives hard. Basically CSS is inherently a no boundary language - it gives the tools to shape our own boundaries - which makes it easy to use but also makes it easy to create a mess **because there is no internal organizing factor**.
...
The third is actually very easy to work with; to put it bluntly, it is the language of CSS, and can be learned. Over time. But because starting is easy, it doesn't mean there won't be subtlety and interest.
...
If we apply our mental model, we can make this statement: "frameworks create consistent boundaries and provide structure to write and think about CSS". This means they can solve a lot of the hard parts of working with CSS, but as per the main observation of the mental model, by adding boundaries they also introduce the costs of crossing boundaries. A couple of examples by way of demonstration are:

- Complexity: while they clarify how we should use CSS, CSS is huge and flexible, consequently they either have to be equally large and flexible or be much reduced. As a result complexity arises in one of 2 ways: Either they become complex themselves - e.g. their syntax becomes opaque, `<div class="Bgc(#0280ae.5) H(90px) D(ib)--sm W(25%)--sm"></div>` (from the Atomic CSS framework website). Alternatively they are too limited and mean that we also write CSS outside of their framework, which adds another boundary and complexity.
- Unclear boundaries: ironically their clear rules can obscure other boundaries within their context - for me the most problematic thing with frameworks, albeit rather nuanced, is that they can engender a mental model where HTML and CSS are equally important - that it is okay to write HTML to fit the needs of the CSS classes that are needed rather than the need of the content. This not limited to frameworks; it is just something I see very often in codebases where frameworks have been used.

Clearly frameworks successfully achieve the ordering of the CSS environment, nevertheless there are also costs to consider.

### Conventions

The other option, Conventions, are kind of like a "Do It Yourself Framework". They can vary in their scope but they all represent organizing principles that overlay boundaries on CSS. BEM for example is a way of marking boundaries through naming, while OOCSS outlines an entire philosophy of structure. They are more simple than frameworks and come with less cruft, but on the other side of the coin, developers need to then set everything up themselves and they lose out on the battle tested code and practices. There are fewer boundaries and developers have control over these boundaries.
...
Frameworks use and formalize one or more Convention into tooling and code structure. With Conventions developers choose instead to do that work themselves. Neither Frameworks nor Conventions resulted in small stylesheets, but they did help in reasoning about them. ITCSS, in my experience, was most successful at resolving the problems of "append only" stylesheets.
...
One of the key principles of ITCSS is that it distinguishes **7 types of CSS style** and separates the CSS into these sections (called layers). What the layers are is a little academic once you understand the principle, but the ones recommended in ITCSS are ordered from most general (wide, top of the inverted triangle) to most specific (sharp, bottom of the triangle).

![modified ITCSS from the ITCSS website](/assets/blog/tof/ITCSS.png)

My teams and I have used a reduced version of this very successfully over a few years. We have adjusted it and learned from using it. We've even managed some projects across multiple branding changes - one of the arguments for Separation of Concerns that is often rejected as not a real life scenario.

This is not an advert for ITCSS though it might seem it 🤨, rather the aim is to understand why it is successful. In application of the boundary mental model, it is because it draws boundaries that aren't at odds with actual usage or unnatural to the domain. Consequently it feels as if it works in harmony with CSS, and existing ways of using it - as if there are no boundaries. Furthermore, because of this, it isn't necessary to learn whole new syntaxes or frameworks.
...
"CSS-in-JS" is a catch all term to describe a number of approaches to, what I think of as a natural evolution of CSS that was driven by the forces of tooling (pre/post processing and bundling) and the move towards "componentising" UI such as React and Custom Components.
...
Having stepped back and evaluated CSS-in-JS in context, it is easier to make decisions about which tool/CSS-in-JS library is right for you and to notice some of the gaps in tooling that might make life easier.

### Which flavour? 🍦

The choice of CSS-in-JS will come with all its own boundaries and costs, and it should be evaluated for your use case. In my case, although it isn't particularly popular, I tend to favour [CSS modules](https://github.com/css-modules/css-modules). I do this having considered the costs because:
...
2. The convention for CSS modules is to have a separate CSS file from the JS. The style classes are exposed as named exports from the module that can be imported into a JS file and used like a JS object. This means that CSS files can be worked without affecting the JS while still maintaining the explicit CSS dependency boundary relationship. Admittedly, one loses out on the convenience of the CSS actually being in the JS, but gain in simplicity and fault isolation.

Other CSS-in-JS options provide other benefits, but by itself this one choice mitigates 3 of the issues noted about CSS-in-JS (syntax, fault isolation and buy-in), so from that point of view it makes sense.
...
Having made these 2 choices, the only costs of using CSS-in-JS are the related costs of code duplication and isolation leading to needing to override styles. That may be enough. Nevertheless, I have finally come to the crux of the matter, the promised holistic approach to the problems that have existed from the beginning and to try to learn the lessons of 20+ years of web development experience.

## ITCSS it again

I'm re-looking to ITCSS for its success in this area; given that it is one of the few options that don't create new problems, I hope to learn its lessons and make some new observations.

The first observation is that, over time that the 7 "layers" or types of CSS defined in ITCSS fall fairly
...
(elements and context on each page). I've modified this diagram to reflect these:
...
Some CSS-in-JS approaches have the concept of "themes". Themes are a set of shared styles that can be used throughout an application - the "Settings" layer in the figure above. Let's evaluate 2 ways of providing a theme with the mental model; one CSS-in-JS solution and one adapted from the observations above.
...
1. There is the JS/CSS language boundary
2. Related, there is a possible boundary between JS devs and CSS devs (it may depend on your team)
3. The JSX `<button>` vs HTML `<button>` boundary (handled by React)
4. There is the `Button = styled.button`/JSX `<button>` boundary. Handled by
   ...
   for some values
5. There is the theme, which is a JS Object (has a closure/boundary around it).
...
At first glance these don't seem like real boundaries, so it is worth taking the time to reason through a couple. For example, the first couple of boundaries are about a skillset/team boundary. This doesn't make sense unless you look at the files from the perspective of understanding and cognitive load. To a "CSS dev", the majority of that
...
costly, both in training and in consequences of mistakes (because JS isn't as forgiving
as CSS).

Another example from point 1: JS/CSS language is represented by that (initially) unusual
...
encapsulate components/JSX with styles AND one can use CSS syntax instead
...
"Componentize" UI, VDOM, reactive rendering, etc. Tons of value, but the cost is a minimum bundle
...
This last point is what led me to this simple thesis: leverage CSS for what CSS does well and CSS-in-JS for what it does well while protecting the fundamental philosophies and boundaries of each.

Basically leverage the existing boundaries around those 2 types of style. I rather pompously call it the "Triangle of Forces", which represents 3 forces in balance, because it is heavily inspired by ITCSS's layers, it uses CSS-in-JS and it proposes some new conventions for using CSS-in-JS.

## Enter the Triangle of forces 🔻

On the face of it ToF is a split in the styles of an application or website, where there are some styles that are relatively static and get used across multiple projects and some that are specific to an implementation. In the figure above, the stronger the colour the more changeable the layer, especially across projects.
...
Styled-Components except that it is written in CSS and is therefore global - there are no ThemeProviders, no imports, no boundaries and no costs... This is safe because we are not declaring styles. This is the only place that this type of global CSS variable is declared
...
rather than cut across and we can come up with solutions that are better than just
